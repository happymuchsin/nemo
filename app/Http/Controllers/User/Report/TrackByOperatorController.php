...
class TrackByOperatorController extends Controller
{
    ...
    {
        $page = 'user_report_track_by_operator';
        $title = 'USER REPORT TRACK BY OPERATOR';

        HelperController::activityLog('OPEN USER REPORT TRACK BY OPERATOR', null, 'read', $request->ip(), $request->userAgent());

        return view('User.Report.TrackByOperator.index', compact('title', 'page'));
    }
