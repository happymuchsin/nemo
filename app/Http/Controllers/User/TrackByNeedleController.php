...
class TrackByNeedleController extends Controller
{
    ...
    {
        $page = 'user_track_by_needle';
        $title = 'USER TRACK BY NEEDLE';

        HelperController::activityLog('OPEN USER TRACK BY NEEDLE', null, 'read', $request->ip(), $request->userAgent());

        return view('User.TrackByNeedle.index', compact('title', 'page'));
    }
